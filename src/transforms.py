...
class RandomIoUCrop(nn.Module):
    ...
    def forward(
        ...
        # _, orig_h, orig_w = F.get_dimensions(image) # NOTE: nazekaugokanai
        _, orig_h, orig_w = image.size()
...
class RandomPhotometricDistort(nn.Module):
    ...
    def forward(
        ...
        if r[6] < self.p:
            # channels, _, _ = F.get_dimensions(image) # NOTE: nazekaugokanai
            channels, _, _ = image.size()
...
class ScaleJitter(nn.Module):
    ...
    def forward(
        ...
        # _, orig_height, orig_width = F.get_dimensions(image) # NOTE: nazekaugokanai
        _, orig_height, orig_width = image.size()
...
class FixedSizeCrop(nn.Module):
    ...
    def forward(self, img, target=None):
        # _, height, width = F.get_dimensions(img) # NOTE: nazekaugokanai
